...
- `torch/distributed.py` — `DistributedStrategy` implementations (single device, DDP, FSDP2) owning model wrapping, initial weight sync, and checkpoint state dicts, plus the `sync_gate` used by generated learners
