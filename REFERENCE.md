...
Every mesh axis is `AxisType.Auto` unless `model_axis_mode` is `"explicit"`, which types the whole mesh — Flax refuses a mesh whose axes do not all have the same type (`Mesh must have all axes as Explicit or all axes as Auto`), so the data axis takes whichever type the model axis has, and `nnx.with_partitioning` initializers work under either mode. Under the Auto default the compiler inserts a row-parallel layer's reduction itself, a plain layer parallelizes with no model-template change, and a model carrying no sharding annotation at all traces — which an Explicit data axis did not allow, since an Explicit axis demands an `out_sharding` wherever a replicated array meets a sharded one and those points sit inside library code no template can annotate: `nnx.Embed`'s gather of a replicated table with data-sharded indices and the class-token `jnp.concatenate` both raise `ShardingTypeError` under one (the H200 amendment to [`docs/adr/0022`](docs/adr/0022-tensor-parallel-strategies-on-a-second-mesh-axis.md)). Under `"explicit"`, `wrap()` verifies that every layer a `row` rule matched carries a `dot_general` hook of its own, raising with the layer path and the template line that fixes it. That hook is **`dot_general_out(*spec)`** (in [`flax/utils.py`](src/structcast_model/flax/utils.py), re-exported from `structcast_model.flax`) — a `jax.lax.dot_general` placing its result on `PartitionSpec(*spec)`, reached from a model template as `dot_general: "eval: dot_general_out('data', None)"`. It is a closure rather than a `functools.partial` because Flax passes `out_sharding=` explicitly on every call to the hook, silently overriding a keyword a partial bound.
