...
- **EMA is refused under sharding** — a learner declaring `EMA` does not build under FSDP2 or tensor parallelism. The models reach the learner already wrapped, and a [`torch.optim.swa_utils.AveragedModel`](https://docs.pytorch.org/docs/stable/optim.html#stochastic-weight-averaging) has no copy to take of a module whose parameters are `DTensor` shards, so the generated `__init__` raises a `ValueError` naming the model rather than degrading silently ([`docs/adr/0021`](docs/adr/0021-ema-as-learner-declared-shadow-models.md)). Drop the `EMA` entry — on the showcase templates that is `-p "SHARED: {ema: false}"` — or train that model under DDP or a single device, which keep whole parameters.
