...
of the generated language-model example, on 4×H200 (driver 595.71.05, container
`docker/train.dockerfile`, torch 2.11.0+cu130).

## Setup

- Model: generated `cfg/torch/models/SmallLanguageModel.yaml` `small` (dim 512, 8 heads, 8 blocks,
  configuration-defined causal attention with rotary position embedding, vocab 65, sequence 256) —
  `named_modules()` paths `backbone.block0..7`.
...
| ----------- | ----------------- | ---------- | --------- |
| single-root | 0                 | 1512.9     | 3.685     |
| per-block   | 8 (`backbone.block0..7`) | **1379.3** (−8.8%) | 3.685 |
...
Tiny Shakespeare via `TinyShakespeareLoader`): epochs 1→2 val_ce_loss 2.161 → 1.888 (the earlier
learned-position variant reached 2.437, and its resume from the epoch-2 DCP training state
continued at epoch 3 with 2.290). Per-block in-place compile (`nn.Module.compile` on each matched
block) ran in the same commands.
...
- FSDP2's checkpoint path refuses optimizer proxies (`AdamWWithCosine`), by design: the
  language-model learner switched to a native `torch.optim.AdamW` built by
