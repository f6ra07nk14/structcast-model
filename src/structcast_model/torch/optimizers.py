...
def _create_opt(
    ...
    **kwargs: Any,
    ...
    if layer_decay is not None:
        print(f"Using layer decay: {layer_decay}")
    if weight_decay > 0.0:
        print(f"Using layer decay with weight decay: {weight_decay}")
...
def _create_native_scheduler(
    ...
    **kwargs: Any,
    ...
    if isinstance(scheduler, lr_scheduler.ReduceLROnPlateau):
        ...
        GLOBAL_CALLBACKS.on_epoch_end.append(lambda i: scheduler.step(i.logs()[criterion], i.epoch))
        if has_lr_scale:
            GLOBAL_CALLBACKS.on_epoch_end.append(lambda _: _set_lr_scale(optimizer))
        ...
        GLOBAL_CALLBACKS.on_update.append(lambda i: scheduler.step((i.update - 1) / updates_per_epoch))
        if has_lr_scale:
            GLOBAL_CALLBACKS.on_update.append(lambda _: _set_lr_scale(optimizer))
    else:
        GLOBAL_CALLBACKS.on_epoch_end.append(lambda _: scheduler.step())
        if has_lr_scale:
            GLOBAL_CALLBACKS.on_epoch_end.append(lambda _: _set_lr_scale(optimizer))
...
def _create_timm_scheduler(optimizer: Optimizer, criterion: str, name: str, **kwargs: Any) -> None:
    ...
    GLOBAL_CALLBACKS.on_update.append(lambda i: scheduler.step_update(i.update, i.logs()[criterion]))
    GLOBAL_CALLBACKS.on_epoch_end.append(lambda i: scheduler.step(i.epoch, i.logs()[criterion]))
