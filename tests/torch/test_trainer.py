...
def test_training_step_tensor_outputs_override_model_kwargs() -> None:
    ...
    class _TensorLoss(Module):
        def forward(self, **kwargs: Any) -> dict[str, torch.Tensor]:
            received.update(kwargs)
            return {"loss": torch.tensor(0.0)}

    class _OutputModel(Module):
        def forward(self, **kwargs: Any) -> dict[str, Any]:
            # Emits key named "m" to shadow the model itself
            return {"m": torch.tensor(42.0)}

    model = _OutputModel()
    step = TrainingStep(models=["m"], losses=_TensorLoss())
    step({"x": torch.zeros(1)}, m=model)
    # The tensor output "m" should shadow the model object in losses kwargs
    assert isinstance(received.get("m"), torch.Tensor)
